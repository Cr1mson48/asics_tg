    ...
    try:
        if message.text == "Изменить цену за электроэнергию":
            bot.send_message(message.from_user.id,
                             "Напишите цену на электроэнергию в выбранной валюте (kWh). Пример: 3")
            bot.register_next_step_handler(message, electricity_ru)
        asicw = message.text
        if asicw == 'В главное меню':
            bot.send_message(message.from_user.id, "Вы в главном меню", reply_markup=ru)
        else:
            db_sess = Session()
            us = db_sess.query(User).filter(User.id == message.from_user.id).first()
            side = ''
            try:
                print(asics_s[asicw + '\xa0'][0])
                print('ОНО')
                if 'TH/s' in asics_s[asicw + '\xa0'][0]:
                    side = 'TH'
                elif 'MH/s' in asics_s[asicw + '\xa0'][0]:
                    print('ДА')
                    side = 'MH'
                elif 'KH/s' in asics_s[asicw + '\xa0'][0]:
                    side = 'KH'
                elif 'GH/s' in asics_s[asicw + '\xa0'][0]:
                    side = 'GH'
                elif 'H/s' in asics_s[asicw + '\xa0'][0]:
                    side = 'H'
            except Exception:
                print(asics_s[asicw][0])
                print('ОНО')
                if 'TH/s' in asics_s[asicw][0]:
                    side = 'TH'
                elif 'MH/s' in asics_s[asicw][0]:
                    print('ДА')
                    side = 'MH'
                elif 'KH/s' in asics_s[asicw][0]:
                    side = 'KH'
                elif 'GH/s' in asics_s[asicw][0]:
                    side = 'GH'
                elif 'H/s' in asics_s[asicw][0]:
                    side = 'H'
...
'''
            try:
                if asics_s[asicw + '\xa0'][2] == 'litecoin':
                    text2 = litecoin2(asics_s[asicw + '\xa0'][2], asics_s[asicw + '\xa0'][0].split(' ')[0], asics_s[asicw + '\xa0'][1], us.power, side, rub=rub)
                else:
                    text2 = main(asics_s[asicw + '\xa0'][2], asics_s[asicw + '\xa0'][0].split(' ')[0], asics_s[asicw + '\xa0'][1], us.power, side, rub=rub)

            except Exception:
                if asics_s[asicw][2] == 'litecoin':
                    text2 = litecoin2(asics_s[asicw][2], asics_s[asicw][0].split(' ')[0], asics_s[asicw][1], us.power, side, rub=rub)
                else:
                    text2 = main(asics_s[asicw][2], asics_s[asicw][0].split(' ')[0], asics_s[asicw][1], us.power, side, rub=rub)
...
'''
            text3 = text + '\n' + text2 + '\n' + text4
            bot.send_message(message.from_user.id, text3, reply_markup=mainer)
            print(message.text)
            bot.register_next_step_handler(message, asic)
    except Exception:
        pass
